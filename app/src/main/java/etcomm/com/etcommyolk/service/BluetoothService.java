...
public class BluetoothService extends Service  {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        // TODO Auto-generated method stub
        logI("onStartCommand");
        myback = new BlueDataCallback(EtcommApplication.mCon);
        if (devi == null) {
            devi = getWristBand();
        }
    ...
    private WristBandDevice getWristBand() {
        WristBandDevice device = WristBandDevice.getInstance(EtcommApplication.mCon);
    ...
    void sync0x29_data() {
        ...
        WriteOneDataTask task = new WriteOneDataTask(EtcommApplication.mCon, data);
    ...
    class BlueDataCallback extends BluetoothDataParseBiz {
        ...
        public void connectStatue(boolean isConnect) {
            ...
            if (isConnect) {
                ...
                GlobalSetting.getInstance(EtcommApplication.mCon).saveBlueConn(isConnect);
                sync0x29_data();
        ...
        public void onWristBandFindNewAgreement(WristBand dev) {
             ...
             */
            if (!MineDeviceActivity.devs.contains(dev) && StringUtils.startsWithIgnoreCase(dev.getName(), "Braceli5")) {
                /**
                 * 静态调用设备列表页面的列表数据并填充数据
                 */
                MineDeviceActivity.devs.add(dev);
                /**
                 * 通知设备列表页面ListView 刷新
                 */
                ...
                // }
                // mAdapter.notifyDataSetChanged();
            }else if(!MineDeviceActivity.devs.contains(dev) && StringUtils.startsWithIgnoreCase(dev.getAddress(), "F4:06:A5")) {
                /**
                 * 静态调用设备列表页面的列表数据并填充数据
                 */
                MineDeviceActivity.devs.add(dev);
...
//                    Intent data_0x29 = new Intent();
    ...
    void set0x29_data(String str) {
        ...
        GlobalSetting.getInstance(EtcommApplication.mCon).saveBlueData(str);
    }
