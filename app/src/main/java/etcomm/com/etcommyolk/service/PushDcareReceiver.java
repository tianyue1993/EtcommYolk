...
import etcomm.com.etcommyolk.fragment.BaseFragment;
import etcomm.com.etcommyolk.utils.GlobalSetting;
import etcomm.com.etcommyolk.utils.NotifyUtil;
		...
		switch (bundle.getInt(PushConsts.CMD_ACTION)) {
			case PushConsts.GET_MSG_DATA:
				...
				if (payload != null) {
					...
					if (BaseFragment.isActive) {// app正在运行,发送广播
...
//					AppSharedPreferencesHelper.setHaveReceiveUnReadData(true);
					...
					if (type == 2) {
						// 发送通知
						GlobalSetting.getInstance(context).setHaveReceiveUnReadData(true);
