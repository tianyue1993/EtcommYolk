...
public class NewsListHandler extends JsonResponseHandler {

    public NewsListHandler() {
        this(DEFAULT_CHARSET);
    }

    public NewsListHandler(String encoding) {
        super(encoding);
    }
                    ...
                    public void run() {
                        try {
                            if (response.has("code") && !response.isNull("code")) {
                                ...
                                if (code == 0) {
                                    onSuccess(JSON.parseObject(response.toString(), NewsList.class));
                                } else if (code == 10000) {
                                    exceptionCode();
                                } else {
