 ...
 */
public class DefaultAvatorHandler extends JsonResponseHandler  {


    public DefaultAvatorHandler() {
        this(DEFAULT_CHARSET);
    }

    public DefaultAvatorHandler(String encoding) {
        super(encoding);
    }
                    ...
                    public void run() {
                        try {
                            if (response.has("code") && !response.isNull("code")) {
                                ...
                                if (code == 0) {
                                    onSuccess(JSON.parseObject(response.toString(), DefaultAvatorEntity.class));
                                } else if (code == 10000) {
                                    exceptionCode();
                                } else {
