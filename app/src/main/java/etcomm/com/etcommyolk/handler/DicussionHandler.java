...
public class DicussionHandler extends JsonResponseHandler {

    public DicussionHandler() {
        this(DEFAULT_CHARSET);
    }

    public DicussionHandler(String encoding) {
        super(encoding);
    }
                    ...
                    public void run() {
                        try {
                            if (response.has("code") && !response.isNull("code")) {
                                ...
                                if (code == 0) {
                                    onSuccess(JSON.parseObject(response.toString(), Discussion.class));
                                } else if (code == 10000) {
                                    exceptionCode();
                                } else {
