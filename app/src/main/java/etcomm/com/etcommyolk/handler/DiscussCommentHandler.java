...
public class DiscussCommentHandler extends JsonResponseHandler {

    /**
     * 用于返回值content仅仅只有一个string数据格式
     */
    public DiscussCommentHandler() {
        this(DEFAULT_CHARSET);
    }

    public DiscussCommentHandler(String encoding) {
        super(encoding);
    }
                        ...
                        try {
                            if (response.has("code") && !response.isNull("code")) {
                                ...
                                if (code == 0 || code == 30005) {
                                    onSuccess(JSON.parseObject(response.toString(), DiscussionComment.class));
                                } else if (code == 10000) {
                                    exceptionCode();
                                } else {
                                    //请求异常,弹出提示
