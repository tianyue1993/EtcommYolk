...
public class FindListHandler extends JsonResponseHandler {

    public FindListHandler() {
        this(DEFAULT_CHARSET);
    }

    public FindListHandler(String encoding) {
        super(encoding);
    }
                    ...
                    public void run() {
                        try {
                            if (response.has("code") && !response.isNull("code")) {
                                ...
                                if (code == 0) {
                                    onSuccess(JSON.parseObject(response.toString(), FindList.class));
                                } else if (code == 10000) {
                                    exceptionCode();
                                } else {
