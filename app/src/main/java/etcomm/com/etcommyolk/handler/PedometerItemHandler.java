 ...
 */
public class PedometerItemHandler extends JsonResponseHandler {

    /**
     * 用于返回值content仅仅只有一个string数据格式
     */
    public PedometerItemHandler() {
        this(DEFAULT_CHARSET);
    }

    public PedometerItemHandler(String encoding) {
        super(encoding);
    }
                        ...
                        try {
                            if (response.has("code") && !response.isNull("code")) {
                                ...
                                if (code == 0 || code == 30005) {
                                    onSuccess(JSON.parseObject(response.toString(), PedometerItem.class));
                                } else if (code == 10000) {
                                    exceptionCode();
                                } else {
