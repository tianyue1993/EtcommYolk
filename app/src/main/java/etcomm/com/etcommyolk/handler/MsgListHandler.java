...
/**
 * Created by Hello on 2017/1/3.
 */
public class MsgListHandler extends JsonResponseHandler {

    /**
     * 用于返回值content仅仅只有一个string数据格式
     */
    public MsgListHandler() {
        this(DEFAULT_CHARSET);
    }

    public MsgListHandler(String encoding) {
        super(encoding);
    }
                        ...
                        try {
                            if (response.has("code") && !response.isNull("code")) {
                                ...
                                if (code == 0 || code == 30005) {
                                    onSuccess(JSON.parseObject(response.toString(), MsgListItems.class));
                                } else if (code == 10000) {
                                    exceptionCode();
                                } else {
                                    //请求异常,弹出提示
