...
public class QuitGroupHandler extends JsonResponseHandler {
    ...
    public QuitGroupHandler(String encoding) {
        super(encoding);
    }
                        ...
                        try {
                            if (response.has("code") && !response.isNull("code")) {
                                ...
                                if (code == 0 || code == 30005) {
                                    onSuccess(JSON.parseObject(response.toString(), GoodGroup.class));
                                } else if (code == 10000) {
                                    exceptionCode();
                                } else {
                                    //请求异常,弹出提示
