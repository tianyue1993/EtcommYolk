...
public class TopicMemberHandler extends JsonResponseHandler {

    /**
     * 用于返回值content仅仅只有一个string数据格式
     */
    public TopicMemberHandler() {
        this(DEFAULT_CHARSET);
    }

    public TopicMemberHandler(String encoding) {
        super(encoding);
    }
                    ...
                    public void run() {
                        try {
                            if (response.has("code") && !response.isNull("code")) {
                                ...
                                if (code == 0 ) {
                                    onSuccess(JSON.parseObject(response.toString(), TopicMember.class));
                                } else if (code == 10000) {
                                    exceptionCode();
                                } else {
                                    //请求异常,弹出提示
