...
public class FindHomeHandler extends JsonResponseHandler {

    public FindHomeHandler() {
        this(DEFAULT_CHARSET);
    }

    public FindHomeHandler(String encoding) {
        super(encoding);
    }
                    ...
                    public void run() {
                        try {
                            if (response.has("code") && !response.isNull("code")) {
                                ...
                                if (code == 0) {
                                    onSuccess(JSON.parseObject(response.toString(), FindHome.class));
                                } else if (code == 10000) {
                                    exceptionCode();
                                } else {
